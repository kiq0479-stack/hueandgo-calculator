...
while (( iter <= MAX_ITERATIONS )); do
  ...
  # Autonomous mode system prompt - DISABLED due to --append-system-prompt bug in Claude CLI
  # TODO: Re-enable when Claude CLI fixes --append-system-prompt with --output-format stream-json
  ...
  ui_waiting
  claude_out=""
  # Write prompt to temp file and pass via stdin (workaround: claude -p "$var" fails
  # when invoked from Claude Code Bash tool or other non-TTY environments)
  prompt_file="$(mktemp)"
  ...
  if [[ "$WATCH_MODE" == "verbose" ]]; then
    ...
    if [[ -n "$TIMEOUT_CMD" ]]; then
      $TIMEOUT_CMD "$WORKER_TIMEOUT" "$CLAUDE_BIN" "${claude_args[@]}" < "$prompt_file" 2>&1 | tee "$iter_log" | "$SCRIPT_DIR/watch-filter.py" --verbose
    else
      "$CLAUDE_BIN" "${claude_args[@]}" < "$prompt_file" 2>&1 | tee "$iter_log" | "$SCRIPT_DIR/watch-filter.py" --verbose
    fi
    ...
    [[ ! " ${claude_args[*]} " =~ " --verbose " ]] && claude_args+=(--verbose)
    if [[ -n "$TIMEOUT_CMD" ]]; then
      $TIMEOUT_CMD "$WORKER_TIMEOUT" "$CLAUDE_BIN" "${claude_args[@]}" < "$prompt_file" 2>&1 | tee "$iter_log" | "$SCRIPT_DIR/watch-filter.py"
    else
      "$CLAUDE_BIN" "${claude_args[@]}" < "$prompt_file" 2>&1 | tee "$iter_log" | "$SCRIPT_DIR/watch-filter.py"
    fi
    claude_rc=${PIPESTATUS[0]}
    ...
    [[ ! " ${claude_args[*]} " =~ " --verbose " ]] && claude_args+=(--verbose)
    if [[ -n "$TIMEOUT_CMD" ]]; then
      $TIMEOUT_CMD "$WORKER_TIMEOUT" "$CLAUDE_BIN" "${claude_args[@]}" < "$prompt_file" > "$iter_log" 2>&1
    else
      "$CLAUDE_BIN" "${claude_args[@]}" < "$prompt_file" > "$iter_log" 2>&1
    fi
    claude_rc=$?
    claude_out="$(cat "$iter_log")"
  fi
